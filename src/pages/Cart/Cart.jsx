...
const CartItem = ({ game, updateQuantity, removeFromCart }) => {

    const price = game.price || 0;

    return (
        <div className="d-flex border rounded p-2 mb-3">
            ...
            <div className="h-100 w-100">
                <div className="text-start ms-2 row d-flex flex-row w-100 h-75 mb-4" style={{ margin: "-2px 0px 0px 0px" }}>
                    ...
                    <div className="col-4 align-items-center text-end">
                        <span className="text-black fw-medium text-end me-2 my-1" style={{ fontSize: "19px" }}>{price.toLocaleString()}đ</span>
                        <br />
                        <div className="d-flex justify-content-end">
                            ...
                            <span className="text-secondary fw-medium text-end me-2 my-1 text-decoration-line-through">{(price * 10).toLocaleString()}đ</span>
                        </div>
                    </div>
                </div>
                <div className="ms-3 h-25 d-flex border-top align-items-center me-3">
                    <div className="d-flex mt-2 w-50">
                        <svg className="rd Eb Fa" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" style={{ width: "15px" }}>
...
// Component hiển thị tổng thanh toán
const PaymentSummary = ({ totalPrice, current, nextStep, prevStep }) => {
    return (
        <div className="my-3 w-100" id="thanhtoan">
            ...
            <div className="d-flex align-items-center mt-2 w-100 me-5 border-bottom" id="text1">
                ...
                <div className="col-6 d-flex justify-content-end">
                    <p className="text-end text-black fw-normal">{totalPrice.toLocaleString()}đ</p>
                </div>
            </div>
                        ...
                        <div className="col-8 p-4">
                            {totalItems === 0 ? (
                                ...
                                <>
                                    ...
                                    <OverlayTrigger placement="bottom" overlay={<Tooltip id="tooltip-cart">Về trang chủ</Tooltip>} trigger={["hover", "focus"]}>
                                        <Link to="/" style={{ display: "block", textAlign: "center" }}>
                                            <Image src="https://cdn-icons-png.flaticon.com/512/107/107831.png" alt="Empty Cart" width={150} className="mb-3 mt-5" style={{ cursor: "pointer", margin: 0, padding: 0 }} />
                                        </Link>
                                    </OverlayTrigger>
                                </>
                            ) : (
                                ...
                                <div className="">
                                    ...
                                    {cart.map((game) => (
                                        <CartItem key={game.id} game={game} updateQuantity={updateQuantity} removeFromCart={removeFromCart} />
                                    ))}
                                </div>
                            )}
                        </div>
                        <Row className="mt-5 col-4">
                            <Col className="d-flex flex-column align-items-start">
                                ...
                                <div className="mt-2" style={{ textAlign: "center" }}>
                                    ...
                                    <p className="text-muted fw-normal mt-1">Quét mã QR để tặng quà</p>
                                </div>
                                {totalItems > 0 && <PaymentSummary totalPrice={totalPrice} current={current} nextStep={nextStep} prevStep={prevStep} />}
                            </Col>
                        </Row>
                    </Container>
                </div>
